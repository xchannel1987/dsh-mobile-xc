#!/usr/bin/env node
/* dump 0.3.9 卡：YyYd_a_card 卡壳 + 展开后 body 行 */
...
async function main(){
  const browser=findBrowser();const port=7500+Math.floor(Math.random()*90);const ud=mkdtempSync(join(tmpdir(),'dsh-xc-sty3-'))
  ...
  try{
    ...
    const card = await evalJs("(()=>{var el=document.querySelector('[data-xc-card]');if(!el)return null;var head=el.querySelector('.YyYd_a_header');if(head)head.click();return {cls:el.className, headBtn:!!head, bodyBefore:el.querySelector('.YyYd_a_body')?true:false}})()")
    await new Promise(x=>setTimeout(x,600))
    const after = await evalJs("(()=>{var el=document.querySelector('[data-xc-card]');if(!el)return null;return {cls:el.className, hasBody:!!el.querySelector('.YyYd_a_body'), rows:el.querySelectorAll('[data-xc-row]').length, names:[...el.querySelectorAll('.YyYd_a_name')].map(function(n){return n.textContent})}})()")
    console.log('点击头部前:', JSON.stringify(card))
