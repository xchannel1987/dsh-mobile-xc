#!/usr/bin/env node
/* 验证：better-sidebar 右抽屉打开时 .dsh-xc-ham 隐藏（v0.1.7） */
...
async function main(){
  const browser=findBrowser();const port=9600+Math.floor(Math.random()*90);const ud=mkdtempSync(join(tmpdir(),'dsh-xc-bs-'))
  ...
  try{
    ...
    for(let i=0;i<60;i++){if(await evalJs("!!document.querySelector('[data-shell-overlay]')"))break;await new Promise(x=>setTimeout(x,500))}
    const ham = () => evalJs("(()=>{const h=document.querySelector('.dsh-xc-ham');return h?getComputedStyle(h).display:'(no-ham)'})()")
    console.log('ham display(初始):', await ham())
    // 点 better-sidebar 切换按钮
    const ok = await evalJs("(()=>{const b=document.querySelector('[data-dsh-panel-host] .nArs4W_toggleCluster button, [data-dsh-panel-host] .nArs4W_toggleButton');if(!b)return false;b.click();return true})()")
    console.log('better-sidebar toggle clicked:', ok)
    await new Promise(x=>setTimeout(x,600))
    const panelOpen = await evalJs("!!document.querySelector('[data-dsh-panel-host] .nArs4W_panel:not(.nArs4W_panelHidden)')")
    console.log('better-sidebar 面板打开:', panelOpen)
    console.log('ham display(面板打开后):', await ham())
