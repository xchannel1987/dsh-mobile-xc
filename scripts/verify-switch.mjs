...
async function main(){
  const browser=findBrowser();const port=7300+Math.floor(Math.random()*90);const ud=mkdtempSync(join(tmpdir(),'dsh-xc-sw2-'))
  ...
  try{
    ...
    await evalJs("(()=>{var c=document.querySelector('[data-xc-card]');if(c)c.querySelector('.YyYd_a_header').click();return !!c})()")
    await new Promise(x=>setTimeout(x,500))
    const info = await evalJs("(()=>{var c=document.querySelector('[data-xc-card]');if(!c)return null;var rows=[...c.querySelectorAll('[data-xc-row]')];return {rows:rows.length,switches:rows.map(function(r){return {cls:r.querySelector('.dsh-xc-switch').className,title:r.querySelector('.dsh-xc-srow-title').textContent,hint:r.querySelector('.dsh-xc-srow-hint').textContent}})}})()")
    console.log('卡内 Switch 行:', JSON.stringify(info, null, 1))
    if (info && info.switches && info.switches.length > 0) {
      console.log('初始首个 on:', info.switches[0].cls.includes('on'))
      await evalJs("(()=>{var c=document.querySelector('[data-xc-card]');var r=c.querySelector('[data-xc-row]');var inp=r.querySelector('input');inp.click();return true})()")
      await new Promise(x=>setTimeout(x,900))
      const after = await evalJs("(()=>{var c=document.querySelector('[data-xc-card]');var r=c.querySelector('[data-xc-row]');var s=r.querySelector('.dsh-xc-switch');return {cls:s.className,checked:r.querySelector('input').checked}})()")
      console.log('切换后:', JSON.stringify(after))
      console.log('结果:', after && after.checked === true && after.cls.includes('on') ? 'PASS：Switch 切换 + on 态正常' : 'FAIL')
    }
