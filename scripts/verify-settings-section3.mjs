...
async function main() {
  ...
  const ud = mkdtempSync(join(tmpdir(), 'dsh-xc-set3-'))
  ...
  try {
    ...
    let ready = false
    for (let i = 0; i < 90; i++) {
      const s = await evalJs("!!document.querySelector('[data-shell-overlay]')")
      if (s === true) { ready = true; break }
    ...
    const open = await evalJs("(()=>{var o=document.querySelector('[data-shell-overlay]');return o? !o.parentElement.hasAttribute('data-sidebar-collapsed') : 'no-overlay'})()")
    console.log('drawer open:', open)
    await evalJs("(()=>{var b=document.querySelector('.VOzbGW_trigger');if(b){b.click();return true}return false})()")
    await new Promise(x => setTimeout(x, 1000))
    console.log('dialog:', await evalJs("!!document.querySelector('[role=\"dialog\"][aria-modal]')"))
    ...
    console.log('dialog 按钮:', JSON.stringify(rows))
    ...
    console.log('移动端区块:', JSON.stringify(labels, null, 1))
