#!/usr/bin/env node
/* 验证 v4：模拟 better-sidebar 面板打开（摘掉 panelHidden）-> 断言 .dsh-xc-ham display:none */
...
async function main() {
  ...
  const ud = mkdtempSync(join(tmpdir(), 'dsh-xc-bs4-'))
  ...
  try {
    ...
    // 摘掉隐藏类（模拟打开）
    await evalJs("(()=>{var p=document.querySelector('[data-dsh-panel-host] .nArs4W_panel');if(p){p.classList.remove('nArs4W_panelHidden');return true}return false})()")
    await new Promise(x => setTimeout(x, 150))
    const ham = await evalJs("(()=>{var h=document.querySelector('.dsh-xc-ham');return h?getComputedStyle(h).display:'(no-ham)'})()")
    const panelHiddenNow = await evalJs("document.querySelector('[data-dsh-panel-host] .nArs4W_panel')?.classList.contains('nArs4W_panelHidden')")
    console.log('面板 hidden:', panelHiddenNow, '| ham display:', ham)
    console.log('期望: 面板打开(hidden=false) 且 ham= none ->', (panelHiddenNow === false) && (ham === 'none') ? 'PASS' : 'FAIL')
